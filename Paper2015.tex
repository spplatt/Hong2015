...
\begin{abstract}
    ...
    Neutron field calculations show close agreement with measurements at both standard and close user positions.
...
The present calculations therefore underestimate measurements by 24\%. 
\begin{figure}[t]
...
The present results underestimate measurements by 28\%.
...
The results are in general agreement although the simulation results are somewhat broader than calculations and the sharp cut-off beyond \SI{11}{\cm} is not fully reproduced by the simulation.
...
\begin{figure}[t]
    ...
    \todo{
        Figure:~\ref{fig:TOFSpectra}: Edit legend, edit y label, make simulated data a curve.
    }
\end{figure}
...
\begin{figure}[t]
    \begin{minipage}{\columnwidth}
        ...
        \todo{
            Fig.~\ref{fig:LethargyplotSUP}: update legend. Make dotted curve dash-dotted.
        }
    \end{minipage}
    \begin{minipage}{\columnwidth}
        \centering
...
It is not surprising that this is unaffected by the improved model geometry as this peak arises from intranuclear cascades in the target.
We continue to investigate this discrepancy and will discuss it further in our final paper.

\begin{figure}[!t]
    \centering
    \includegraphics[width=0.8\columnwidth]{DiffYieldComparedSUPCUP10RADECS.eps}
    \caption{
        Differential neutron spectra at CUP (upper curves) and SUP (lower curves), each with the \SI{10.2}{\cm} collimator.
...
\begin{figure}[t]
    \begin{minipage}{\columnwidth}
        ...
        \todo{Fig~\ref{fig:GammaSpatialDistributionSUP}: consider what to do with this figure.}
    \end{minipage}
    \begin{minipage}{\columnwidth}
...
\begin{figure}[t]
    ...
    \todo{Fig.~\ref{fig:GammaDoseEnergy}: correct y axis units. Consider whether to combine.}
\end{figure}
...
Neutron fluence rate, energy spectrum, time profile and and spatial distribution show good agreement with measurements and independent simulations (using MCNPX).
